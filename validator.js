...
class WordValidator {
    ...
    validatePhrase(input, { debug = false } = {}) {
        ...
        // Normalization: lowercase, normalize unicode, lowercase
        const normalized = input.normalize('NFKD').toLowerCase();
        
        // Tokenization: strip punctuation (keep internal apostrophes), trim, split
        // Pattern matches words that may have internal apostrophes (don't, it's)
        const tokens = normalized
            .replace(/[^\w\s']/g, '') // Remove symbols except apostrophe
            .trim()
            .split(/\s+/);
        ...
        return tokens.every(token => {
            // Reject numeric-only tokens or empty
            if (/^\d+$/.test(token) || token === "") return false;
            return this.checkWord(token);
        });
    }
}
